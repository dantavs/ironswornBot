...
export const rollResupply: Command = {
    data: new SlashCommandBuilder()
        .setName("resupply")
        .setDescription("Roll Resupply move for you character")
    ...
    run: async (interaction) => {
        ...
        if (getChar.character){
            ...
            const moveResultDescriptorMiss = new MoveDescriptor({
                resultName:'Miss',
                resultDescriptor : "You find nothing helpful. Pay the Price."
            })
            
            const moveResultDescriptorWeakHit = new MoveDescriptor({
                resultName:'Weak Hit',
                resultDescriptor : 'Take up to +2 Supply. Suffer -1 Momentum for each.'
            })
            
            const moveResultDescriptorStrongHit = new MoveDescriptor({
                resultName:'Strong Hit',
                resultDescriptor : 'You bolster your resources. Take +2 Supply.'
            })
